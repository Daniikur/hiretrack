...
# ============ Config ============
JWT_ALGORITHM = "HS256"
ACCESS_TTL_MIN = 60 * 24  # 24h for dev convenience
REFRESH_TTL_DAYS = 7
...
async def ai_summarize(payload: SummarizeIn, user=Depends(get_current_user)):
    ...
    if payload.mode == "resume":
        system = (
            "You are a resume analyzer. Extract key skills, years of experience, "
            "and top achievements as concise bullet points."
        )
...
"""
        ...
        system = (
            "You are a job description summarizer. Produce 3-5 crisp bullet points "
            "covering required skills, responsibilities, and notable perks."
        )

        prompt = f"""
Summarize this job description in 3-5 bullets:
...
"""
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"AI request failed: {str(e)[:200]}"
        )
...
async def summarize_jd(app_id: str, user=Depends(get_current_user)):
    ...
    if len(jd.strip()) < 20:
        raise HTTPException(
            status_code=400,
            detail="Job description too short to summarize"
        )
    ...
    try:
        response = client_ai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You summarize job descriptions into crisp bullet points."
                },
                {
                    "role": "user",
                    "content": f'''
Summarize this JD in 3-5 bullets.
Focus on required skills, responsibilities, and perks.
...
'''
        ...
        logger.error(f"summarize_jd: {e}")

        raise HTTPException(
            status_code=500,
            detail=f"AI failed: {str(e)[:200]}"
        )
